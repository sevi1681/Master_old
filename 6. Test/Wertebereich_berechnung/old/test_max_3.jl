# include("Desktop/GitHub/Master_lokal/6. Test/Wertebereich_berechnung/test_max_3.jl")
...
while x_out >= x_in     # bei minimum umgekehrtes Vorzeichen!    # oder nur < ?
 ...
 print("\n befor Range: x_in = ",x_in , "\t", "x_out = ", x_out ,   "\n") 
  
 #--------------------------------------------------------------------------


 x_in_range=range(x_in_prev, stop=x_in, length=10000)
 x_out_range_pos=zeros(size(x_in_range))
 x_out_range_neg=zeros(size(x_in_range))
 
 print("\n  x_in_range : von ",x_in_prev , "\t", " bis ", x_in ,   "\n")
