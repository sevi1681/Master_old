# include("Desktop/GitHub/Master_lokal/8. Mehrere Zustandsraumvariablen/parallel/Gleichstrom_max.jl")
 using ACME
 using Plots

u=[ 1.0 for c in 1:1, n in 1:10 ]
...
circ = @circuit begin
  ...
  c2 = capacitor(47e-9), [1] ⟷ j_in[+],[2] ⟷ gnd    # neue Kapazität (c2 parallel zum Eingang)
  r1 = resistor(1e3), [1] ⟷ c2[1]
...
x_next[n]=model.x[1]                                   #   <----------- model.x[1]  für c2 und model.x[2] für c1      
end                                                    #   c2 ist die neue Kapazität
