...
function calculation_1(Variablen_nummer)
...
#---------------------------------step 1------------------------------------------------------------
    ...
    model = DiscreteModel(circ, 1/fs) 
    model.x[Variablen_nummer]=x_in_min
    ACME.step!(ModelRunner(model,false),y,u_pos,1)
    x_out_pos=model.x[Variablen_nummer]
...
for n in eachindex(x_in_range)
    model = DiscreteModel(circ, 1/fs) 
    model.x[Variablen_nummer]=x_in_range[n]
    ACME.step!(ModelRunner(model,false),y,u_pos,1)
    global x_out_range_pos[n]=model.x[Variablen_nummer]
         
    model = DiscreteModel(circ, 1/fs)  
    model.x[Variablen_nummer]=x_in_range[n]
    ACME.step!(ModelRunner(model,false),y,u_neg,1)
    global x_out_range_neg[n]=model.x[Variablen_nummer]
